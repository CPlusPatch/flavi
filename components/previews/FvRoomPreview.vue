...
<template>
	<NuxtLink :to="`/room/${room.id}`" class="flex flex-row gap-2 duration-200">
		<div class="h-12 w-12 rounded-md shrink-0 overflow-hidden flex items-center justify-center shrink-0 shadow">
		...
		<div class="flex flex-col h-full grow justify-around text-sm">
		...
		<div class="shrink-0 flex items-center justify-center pl-1" v-if="unreadCount > 0">
			<span class="inline-flex items-center rounded-full px-2 py-1 [font-size:0.5rem] font-bold text-red-100 ring-1 ring-inset ring-red-600/80 bg-red-800">{{ unreadCount }}</span>
		</div>
	</NuxtLink>
</template>